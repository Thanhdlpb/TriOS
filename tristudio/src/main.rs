...
async fn run_code(req: web::Json<RunRequest>) -> HttpResponse {
    ...
    match parser.parse() {
        ...
        Err(e) => HttpResponse::Ok().json(RunResponse { output: vec![], error: Some(e) }),
    }
}
...
async fn save_code(req: web::Json<SaveRequest>) -> HttpResponse {
    ...
    match fs::write(&file_path, &req.code) {
        Ok(_) => HttpResponse::Ok().json(serde_json::json!({"message": format!("✅ Đã lưu vào {}", file_path.display())})),
        Err(e) => HttpResponse::Ok().json(serde_json::json!({"message": format!("❌ Lỗi: {}", e)})),
    }
}
...
async fn main() -> std::io::Result<()> {
    ...
    HttpServer::new(|| {
        App::new()
            .service(web::scope("/api").route("/run", web::post().to(run_code)).route("/save", web::post().to(save_code)))
            .service(Files::new("/static", "tristudio/static").show_files_listing())
            .route("/", web::get().to(|| async { HttpResponse::Ok().content_type("text/html; charset=utf-8").body(include_str!("../static/index.html")) }))
    })
    .bind("0.0.0.0:8080")?
    .run()
    .await
